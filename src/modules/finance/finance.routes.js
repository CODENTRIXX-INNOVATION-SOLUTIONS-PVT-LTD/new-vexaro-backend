...
router.get('/wallet', validateRequest({ query: emptyObjectSchema }), c.getMyWallet);
// GET  /api/finance/wallets         — SA/Distributor: list all wallets in scope
router.get('/wallets', requireRole(UserRole.SUPER_ADMIN, UserRole.DISTRIBUTOR), validateRequest({ query: schemas.listQuerySchema }), c.listWallets);
// POST /api/finance/topup           — SA/Distributor: add funds to a user's wallet
router.post('/topup', requireRole(UserRole.SUPER_ADMIN, UserRole.DISTRIBUTOR), validateRequest({ body: schemas.topupSchema }), c.topupWallet);
// POST /api/finance/refund          — SA: manually refund a user's wallet
router.post('/refund', requireRole(UserRole.SUPER_ADMIN), validateRequest({ body: schemas.refundSchema }), c.refundWallet);
// POST /api/finance/transfer-to-merchant — Distributor: transfer funds to merchant wallet
router.post('/transfer-to-merchant', requireRole(UserRole.DISTRIBUTOR), validateRequest({ body: schemas.transferToMerchantSchema }), c.transferToMerchant);

// ── Transactions ─────────────────────────────────────────────────────────────
// GET  /api/finance/transactions    — own transaction history (SA/Dist can add ?userId=)
router.get('/transactions', validateRequest({ query: schemas.listQuerySchema }), c.listTransactions);

// ── COD Management ───────────────────────────────────────────────────────────
// GET  /api/finance/cod             — list COD records (scoped by role)
router.get('/cod', validateRequest({ query: schemas.listQuerySchema }), c.listCOD);
// PATCH /api/finance/cod/:id/remit  — mark COD as remitted → credits merchant wallet
router.patch('/cod/:id/remit', requireRole(UserRole.SUPER_ADMIN, UserRole.DISTRIBUTOR), validateRequest({ params: schemas.financeIdParamsSchema, body: schemas.remitCODSchema }), c.remitCOD);

// ── Settlements ──────────────────────────────────────────────────────────────
// GET  /api/finance/settlements     — list settlements
router.get('/settlements', validateRequest({ query: schemas.listQuerySchema }), c.listSettlements);
// POST /api/finance/settlements     — create a settlement request
router.post('/settlements', requireRole(UserRole.SUPER_ADMIN, UserRole.DISTRIBUTOR), validateRequest({ body: schemas.createSettlementSchema }), c.createSettlement);
// PATCH /api/finance/settlements/:id/process — SA marks as COMPLETED/FAILED
router.patch('/settlements/:id/process', requireRole(UserRole.SUPER_ADMIN), validateRequest({ params: schemas.financeIdParamsSchema, body: schemas.processSettlementSchema }), c.processSettlement);
...
router.patch(
  ...
  validateRequest({ params: schemas.refundRequestIdParamsSchema, body: schemas.processRefundRequestSchema }),
...
router.post('/razorpay/create-order', requireRole(UserRole.MERCHANT, UserRole.DISTRIBUTOR), validateRequest({ body: schemas.createOrderSchema }), rzp.createOrder);
// POST /api/finance/razorpay/verify-payment — Merchant/Distributor: verify signature + credit wallet
router.post('/razorpay/verify-payment', requireRole(UserRole.MERCHANT, UserRole.DISTRIBUTOR), validateRequest({ body: schemas.verifyPaymentSchema }), rzp.verifyPayment);

// ── Payment History ──────────────────────────────────────────────────────────
// GET  /api/finance/payments     — paginated payment history (role-scoped)
router.get('/payments', validateRequest({ query: schemas.paymentListQuerySchema }), rzp.listPayments);
